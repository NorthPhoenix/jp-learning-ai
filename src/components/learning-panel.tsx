...
export function LearningPanel({ isOpen, onClose }: LearningPanelProps) {
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div className="bg-card/95 backdrop-blur-glass animate-in slide-in-from-right fixed top-0 right-0 bottom-0 z-50 w-full max-w-md shadow-2xl duration-300">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                {[
                  ...
                  <div key={i} className="bg-muted/30 flex items-center justify-between rounded-lg px-4 py-3">
                    <div className="flex-1">
                      <p className="text-foreground text-sm font-medium">{item.jp}</p>
                      <p className="text-muted-foreground text-xs">{item.en}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-sakura bg-sakura-light rounded-full px-2 py-1 text-xs">{item.type}</span>
